(function () {
  const stages = [
    ...
    },
    {
      ...
      title: "函数、循环与控制规则",
      chapter: "第 2 章：能源搜救",
      ability: "函数、调用、for、range、if、逻辑、while",
      work: "自动救援任务包"
    },
    {
      id: "stage-3",
  ...
  const lessonRows = [
    ["学习", "启动信号", "地图、角色、运行、顺序、采集", "从起点到宝石，站到正确格子后采集", "第一张任务卡", "line-upload", "学习入口不是记按钮，而是理解程序会按顺序改变世界状态。", "先让学生预测到达宝石需要几步，再运行参考程序，对照日志说出每一步改变了什么。", "学生会知道：采集不是装饰动作，它必须发生在正确位置。", ["空采集", "还没到宝石就采集", "走过头"]],
    ["学习", "方向不是视角", "朝向、左转、右转、视角旋转", "完成含转弯的采集路线", "朝向变化记录", "corner-upload", "屏幕看起来的上下左右不等于角色真实朝向，HUD 才是程序执行依据。", "让学生旋转观察地图，再要求他们写下每次 turn 后 Nova 面向哪里。", "学生会知道：转向改变的是角色状态，不是地图本身。", ["把屏幕右边当成角色右边", "转向后忘记重新判断前方"]],
    ...
    ["学习", "星图坐标站", "Nova 坐标、宝石坐标、坐标变化", "根据坐标依次找到两座宝石并解释转向", "坐标档案", "coordinate-scan", "坐标让位置从直觉变成可记录的数据：x 向右增加，y 向下增加，镜头旋转也不会改变对象坐标。", "记录起点、两座宝石和每次转向后的关键坐标，再说明路线为什么能到达目标。", "学生会知道：坐标是地图任务的数据语言，不会随着观察视角改变。", ["横纵顺序读反", "把镜头方向当坐标方向", "只记终点不记录变化"]],
    ["练习", "多段任务顺序", "分段路线、携带状态、上传", "依次采集宝石 A、宝石 B，再到中继站上传", "分段任务说明", "segment-upload", "复杂任务不是一条无法解释的长程序，而是几个有明确完成条件的子目标。采集会改变携带状态，上传必须发生在两座宝石都已携带之后。", "把程序拆成采集 A、采集 B、前往中继站上传三段，并观察携带状态如何变化。", "学生会知道：多段任务要同时管理目标顺序和携带状态。", ["漏掉第二次采集", "提前上传", "到达中继站却没有上传"]],
    ["创作", "设计一条可解路线", "规则设计、可解性、作者解、反例", "设计一关，并用自己的程序证明它能通关", "规则版本与作者解", "creator-template", "可解不是一句保证。目标、障碍和动作上限必须与一份真实运行成功的作者解同时成立。", "在小模板内选择宝石、放置最多两块障碍并设置动作上限；再修复一份坏规则，用内置失败方案试玩自己的作品。", "学生会知道：规则是否可行要由测试、作者解和关联反例共同证明。", ["只保存截图", "动作上限小于作者解", "目标被规则围住"]],
    ["阶段作品", "第一场独立救援", "顺序、方向、规划、坐标、调试、迁移", "独立完成三个短救援任务并留下关键证据", "第一阶段救援档案", "core-capstone", "阶段作品不增加新动作，而是用分段救援、错误修复和目标变化三项短任务分别验证前七课能力。", "先完成 A、B 与上传的分段任务，再修复陌生错误程序，最后在变化朝向和障碍的地图上重新规划。每项可以单独重试。", "学生会知道：真正掌握是能在陌生任务中重新计划、调试和解释，而不是记住一张长地图。", ["漏掉中间目标", "只看最后失败", "变化后照搬旧路线"]],
    ["学习", "把动作装进工具箱", "函数定义、函数调用、行为复用", "把同一套支路动作定义成函数，并在多处调用", "函数定义与调用证据", "function-route", "函数把一组完整动作命名；定义不会自动运行，只有主程序调用时世界才会改变。", "在 visit_side() 中编辑进入支路、采集、返回并恢复朝向的动作，再在三处入口调用；换图后保持函数体，只调整调用位置。", "学生会知道：同一个学生定义的函数可以在不同位置真实复用。", ["只定义不调用", "复制函数体代替调用", "漏掉一次调用"]],
    ["调试", "一个工具，多处使用", "函数起止约定、调用状态、内部修复", "修好函数，让不同位置的每次调用都可靠", "函数调用状态档案", "function-route", "复用工具需要稳定的入口和出口。函数看似完成一次任务，也可能留下错误朝向并破坏后续调用。", "比较每次 visit_side() 调用前后的坐标与朝向，把恢复动作修在函数内部；最后整体旋转通道，用同一个相对动作函数再次验证。", "学生会知道：可靠函数要在多次调用中持续满足清楚的起止约定。", ["第一次采集成功就停止检查", "在某个调用后打补丁", "只比较位置不比较朝向"]],
    ["学习", "尖刺与能量状态", "能量、尖刺格、护盾、状态面板", "穿越尖刺区并保留足够能量", "能量变化记录", "hazard-line", "状态面板会告诉你每个动作的代价：移动、开盾和踏入尖刺格都会消耗能量。", "每一步后记录能量变化，比较有护盾和无护盾穿越尖刺格的差异。", "学生会知道：程序不仅改变位置，也改变资源状态。", ["尖刺格前没有开盾", "只看路线不看能量"]],
    ["创作", "循环尖刺关", "循环、尖刺、限制条件", "学生设计带尖刺格的循环关", "循环尖刺原创关", "narrow-bridge", "带限制的关卡会迫使学生优化，而不是堆很多动作。", "学生设置最大步数或能量上限，要求同伴用循环压缩路线。", "学生会知道：限制条件能让关卡更有教学价值。", ["限制过松", "尖刺格没有影响策略"]],
    ["学习", "条件判断", "if、前方检测、当前位置检测", "前方尖刺才开盾，前方阻挡才转向", "条件规则卡", "hazard-line", "if 是让程序根据现场情况选择动作，不是固定替代某个按钮。", "让学生先预测条件成立与不成立时分别会执行什么。", "学生会知道：条件判断让程序开始适应地图变化。", ["条件不成立时仍期待动作发生", "检测对象写错"]],
    ["学习", "逻辑守卫", "if else、elif、and/or/not", "组合可走、能量足、不是尖刺", "安全协议卡", "maze-hook", "复杂安全规则通常由几个小判断组合而成。", "把可走、能量足、不是尖刺拆成三张条件卡，再合成安全规则。", "学生会知道：and/or/not 解决的是规则组合问题。", ["and/or 混淆", "not 的范围不清"]],
    ["练习", "while 与上传条件", "while、停止条件、是否携带宝石", "未知距离移动，采集后到中继站上传", "停止条件说明", "repeat-line", "while 的核心是停止条件，条件不清楚就可能少走、过走或死循环。", "让学生写出：什么时候继续走，什么时候停下来采集或上传。", "学生会知道：循环必须有明确结束条件。", ["停止条件漏掉上传", "把 while 当成无限 repeat"]],
    ["阶段作品", "能量与尖刺 Boss 关", "循环、条件、while、能量、上传", "完成综合尖刺区任务", "能量与尖刺任务包", "spiral", "Boss 关要综合检查路线、能量、条件和上传顺序。", "学生提交一张能量变化表，并说明哪一步最容易失败。", "学生会知道：综合任务需要同时管理位置和状态。", ["只优化步数不看风险", "失败卡没有归类"]],
    ...
    ["学习", "把结果交回来", "返回值、布尔函数、结果流向", "返回前方是否安全并用于条件判断", "返回值流向图", "hazard-line", "返回值不会直接改变世界，它把函数算出的结果交回调用处，再由 if 决定动作。", "观察 safe_ahead() 的 True/False 如何流入 if，修复返回方向写反导致的尖刺穿越。", "学生会知道：有些函数负责回答问题，调用者负责使用答案。", ["把返回值当成动作", "函数算出结果但调用处没有使用", "布尔方向写反"]],
    ["学习", "有序任务清单", "列表、索引、遍历、元素", "按距离列表依次处理多个宝石", "多目标清单", "three-targets", "列表把同类任务排成有顺序的清单，索引取出一个元素，遍历则依次处理全部元素。", "先读取第 0 个距离，再遍历剩余距离，补回漏掉的第三段任务。", "学生会知道：列表既能按位置读取，也能作为循环的数据来源。", ["列表漏项", "把第 1 个元素误写成索引 1", "遍历遗漏最后一个元素"]],
    ...
    ["学习", "名字对应规则", "字典、键值、查找、策略映射", "用地形名称查找处理动作", "规则字典", "hazard-line", "字典用有意义的键找到对应规则，适合把地形类型映射到处理策略。", "修复尖刺格 H 对应的动作，并让程序通过键查找决定是否开盾。", "学生会知道：字典把名称与规则配对，查找结果可以直接驱动程序决策。", ["键和值对应错", "查找了不存在的键", "修改规则后没有使用查询结果"]],
    ...
    ["学习", "飞船协作战", "多对象、独立位置、状态、能力边界", "协调 Explorer 与 Spaceship 分工", "协作流程图", "spiral", "多对象不是轮流操纵同一个角色，而是每个实例都保留自己的位置、能量和动作历史。", "修复把采集任务交给飞船的能力错误，让 Nova 与 Carrier 分别承担采集和上传。", "学生会知道：协作首先要把任务交给具有相应能力的对象。", ["对象职责混乱", "只看路线不看能力", "把多个实例当成同一位置"]],
    ...
    ["创作", "数据驱动关卡", "schema、成功条件、限制、可解性", "从地图数据生成原创关卡并校验", "原创关卡 v1", "maze-hook", "原创关卡必须先通过机器可读的 schema 校验，名字、地图、宝石数量和上传条件要彼此一致。", "修复 schema 中错误的宝石数量，生成世界并完成一遍参考解。", "学生会知道：发布前要同时验证数据约定和可运行答案。", ["schema 缺字段", "声明目标与地图不一致", "只有地图没有参考解"]],
    ...
    ["学习", "搜索入门", "线性搜索、筛选、找不到", "找到指定宝石或安全宝石", "搜索记录表", "three-targets", "搜索不是乱找，而是按规则检查候选目标。", "学生按顺序检查多个宝石，标出找到、跳过和找不到的情况。", "学生会知道：搜索要有目标条件，也要处理找不到。", ["找到第一个目标后忘记条件", "没有处理找不到"]],
    ...
    ["练习", "路径还原与代价", "前驱、路径重建、权重、能量代价", "比较最短路线和最低能量路线", "路径对比卡", "hazard-line", "最短步数不一定最低代价，尖刺格会改变路线价值。", "学生比较两条路线的步数和能量消耗，说明选择理由。", "学生会知道：算法评价需要明确指标。", ["只比较步数", "没有记录前驱"]],
    ...
    ["学习", "多目标路线规划", "目标组合、顺序、状态变化", "规划多个宝石的采集顺序", "多目标路线表", "three-targets", "多目标规划要把当前点、已采集集合和剩余目标一起考虑。", "学生列出两种采集顺序，比较步数和风险。", "学生会知道：目标顺序本身就是算法选择。", ["顺序没有理由", "忽略已采集状态"]],
  ...
  const advancedLessonSpecs = {
    ...
    10: {
      ...
      brief: {
        ...
        goal: "先在缩进实验台确认循环范围，再完成两座宝石的折线路线。"
      }
    },
    11: {
      mode: "energy-lab",
      ...
      limit: 7,
      brief: {
        challenge: "穿过辐射格抵达宝石，同时至少保留 4 点能量。直接硬闯虽然更短，但会损失额外能量。",
        intro: "位置不是唯一状态，能量也会决定任务是否成功：",
        bullets: ["每次移动消耗 1 点能量。", "开盾消耗 1 点能量，但能阻止尖刺格造成的额外 4 点损失。", "比较路线时要同时计算步数与能量。"],
        goal: "使用护盾或尖刺检测穿过辐射格，采集后保持能量不少于 4。"
      }
    },
    12: {
      mode: "loop-creator",
      ...
      advancedConfig: {
        ...
        loopHazards: [
          { id: "early", label: "前段尖刺格", x: 3, y: 2, solution: ["move", "ifHazardShield", "move", "repeat4", "collect"] },
          { id: "middle", label: "中段尖刺格", x: 5, y: 2, solution: ["repeat3", "ifHazardShield", "move", "repeat2", "collect"] },
          { id: "late", label: "后段尖刺格", x: 6, y: 2, solution: ["repeat4", "ifHazardShield", "move", "move", "collect"] }
        ]
      },
      brief: {
        challenge: "制作一条带尖刺格的循环路线。选择尖刺格位置，再用循环和护盾写出可通过的参考程序。",
        intro: "有教学价值的原创关卡要让限制真正影响策略：",
        bullets: ["尖刺格必须放在必经路线，而不是装饰区。", "循环负责表达重复移动，护盾负责管理能量风险。", "参考程序必须证明每一种设置都有解。"],
        goal: "选择尖刺位置，让 3D 地图更新，并用不超过 7 条指令验证关卡。"
      }
    },
    13: {
      mode: "condition-lab",
      pythonStudio: {
        ...
        title: "修复尖刺判断",
        taskBadge: "修改第 2 行",
        taskHtml: "把 <code>is_blocked_ahead()</code>（前方是否被阻挡）改成 <code>is_spike_ahead()</code>（前方是否尖刺）。你也可以直接点击标有“本关修改”的代码卡。",
        railEyebrow: "第 2 行需要替换",
        railTitle: "选择正确条件",
        starterSource: ["move()", "if is_blocked_ahead():", "    shield()", "move()", "move()", "move()", "collect()"].join("\n"),
        allowedFunctions: ["move", "turn_left", "turn_right", "shield", "collect", "is_spike_ahead", "is_blocked_ahead"],
        targets: [{ search: "if is_blocked_ahead():", occurrence: 1, hint: "把阻挡检测换成尖刺检测" }],
        templates: [
          { id: "hazard-condition", label: "本关修改", code: "is_spike_ahead()", help: "前方是否尖刺 · 替换错误条件", recommended: true, replaceFrom: "if is_blocked_ahead():", replaceTo: "if is_spike_ahead():" },
          { id: "move", code: "move()", help: "前进一格", snippet: "move()" },
          { id: "collect", code: "collect()", help: "采集宝石", snippet: "collect()" }
        ],
        translations: [["move()", "前进一格"], ["is_blocked_ahead()", "前方是否被阻挡"], ["is_spike_ahead()", "前方是否尖刺"], ["shield()", "开启护盾"], ["collect()", "采集宝石"]]
      },
      start: { x: 1, y: 2 },
      startDir: "E",
      grid: [
    ...
    14: {
      ...
      pythonStudio: {
        ...
        title: "用代码按钮组成安全守卫",
        taskBadge: "选择 2 块代码",
        taskHtml: "不用敲代码。先从左侧选择正确的 <code>if</code> 条件，再加入缩进的 <code>move()</code>。只有“前方可通行、能量大于 3、前方不是尖刺格”同时成立，探测员才能前进。",
        ...
        allowedFunctions: ["move", "turn_left", "turn_right", "collect", "is_path_clear", "is_blocked_ahead", "is_spike_ahead", "energy_remaining"],
        targets: [
        ...
        conceptChecks: [
          ...
          { pattern: "\\bnot\\s+is_spike_ahead\\(\\)", message: "需要用 not 排除尖刺格" },
          { pattern: "energy_remaining\\(\\)\\s*>\\s*3", message: "需要检查剩余能量是否大于 3" }
        ],
        templates: [
          { id: "guard-and", label: "三个条件都成立", code: "if clear and energy and not spike:", help: "AND：必须全部满足", block: true, guided: true, replaceFrom: "# TODO_1: 选择安全条件", replaceTo: "if is_path_clear() and energy_remaining() > 3 and not is_spike_ahead():" },
          { id: "guard-or", label: "任一条件成立", code: "if clear or energy or not spike:", help: "OR：只要一个满足", block: true, guided: true, replaceFrom: "# TODO_1: 选择安全条件", replaceTo: "if is_path_clear() or energy_remaining() > 3 or not is_spike_ahead():" },
          { id: "guard-hazard", label: "只检查尖刺", code: "if not spike:", help: "只判断前方是否安全", block: true, guided: true, replaceFrom: "# TODO_1: 选择安全条件", replaceTo: "if not is_spike_ahead():" },
      ...
      limit: 8,
      brief: {
        challenge: "使用安全守卫绕过起点岩石，并在开阔路段继续前进。守卫要同时检查可通行、能量足够且不是尖刺格。",
        intro: "多个布尔条件可以组合成一条完整安全规则：",
        bullets: ["AND 要求所有条件都为 true。", "OR 只要求其中一个条件为 true。", "NOT 会把一个布尔结果反转，例如 NOT spike（不是尖刺）。"],
        goal: "使用 clear AND enoughEnergy AND NOT spike 的守卫完成路线。"
      }
    },
    15: {
      mode: "while-monitor",
      pythonStudio: {
        ...
        taskBadge: "选择 2 块代码",
        taskHtml: "第一个 <code>while</code> 已经示范了“走到宝石就停止”。不用敲代码：从左侧选择第二个循环的停止目标，再加入缩进的 <code>move()</code>，最后程序会自动执行已有的上传动作。",
        railEyebrow: "按黄色位置依次选择",
        railTitle: "选择循环积木",
        initialFeedback: "先选择正确的 while 停止目标，再选择循环中重复执行的动作。",
        starterSource: ["while not at_gem():", "    move()", "collect()", "", "# TODO_1: 选择第二个 while 的停止目标", "# TODO_2: 选择循环里重复的动作", "", "upload()"].join("\n"),
        allowedFunctions: ["move", "collect", "upload", "at_gem", "at_relay"],
        targets: [
        ...
        templates: [
          ...
          { id: "while-beacon", label: "走到宝石为止", code: "while not at_gem():", help: "未到宝石就继续", block: true, guided: true, replaceFrom: "# TODO_1: 选择第二个 while 的停止目标", replaceTo: "while not at_gem():" },
        ...
        translations: [["while", "当条件成立时重复"], ["not", "还没有达到"], ["at_gem()", "是否在宝石格"], ["at_relay()", "是否在中继站"], ["upload()", "上传任务数据"]]
      },
      start: { x: 1, y: 2 },
      startDir: "E",
      grid: [
      ...
      limit: 6,
      brief: {
        challenge: "宝石和中继站的距离不写在程序里。让 while 根据是否到达目标决定继续移动或停止。",
        intro: "while 的重点不是重复，而是每次重复前都重新检查停止条件：",
        bullets: ["没有到达宝石时继续移动，到达后立即停止。", "采集后，循环目标要从宝石切换为中继站。", "停止条件永远不改变，会造成过走或死循环。"],
    ...
    16: {
      ...
      pythonStudio: {
        ...
        title: "用代码按钮完成能源搜救",
        taskBadge: "组合 6 块代码",
        taskHtml: "不用敲代码。按照三段任务说明，从左侧依次选择路线积木和任务动作：采集第一座宝石、安全采集第二座宝石，最后到中继站上传。",
        railEyebrow: "一次完成一个黄色位置",
        railTitle: "选择任务积木",
        initialFeedback: "从第一个黄色位置开始，依次点击六块代码。编辑器会展示你组合出的完整 Python。",
        starterSource: [
          "# 任务 1：向东走到第一座宝石并采集",
          "# TODO_1A: 选择走到第一座宝石的循环",
          "# TODO_1B: 选择抵达后的动作",
          "",
          "# 任务 2：转向北，安全走到第二座宝石并采集",
          "# TODO_2A: 选择安全路线",
          "# TODO_2B: 选择抵达后的动作",
          "",
          "# 任务 3：转向东，安全走到中继站并上传",
        ...
        allowedFunctions: ["move", "turn_left", "turn_right", "shield", "collect", "upload", "is_spike_ahead", "at_gem", "at_relay"],
        targets: [
          { search: "# TODO_1A: 选择走到第一座宝石的循环", occurrence: 1, hint: "点击走到宝石的循环" },
        ...
        conceptChecks: [
          ...
          { pattern: "\\bif\\s+is_spike_ahead\\(\\)", message: "综合程序需要用 if 判断尖刺格" },
          { pattern: "\\bshield\\(\\)", message: "综合程序需要在尖刺格前开启护盾" }
        ],
        templates: [
          { id: "task1-route", label: "任务 1 · 走到宝石", code: "while not at_gem():\n    move()", help: "重复前进直到宝石", block: true, guided: true, replaceFrom: "# TODO_1A: 选择走到第一座宝石的循环", replaceTo: "while not at_gem():\n    move()" },
          { id: "task1-collect", label: "任务 1 · 采集", code: "collect()", help: "采集第一座宝石", block: true, guided: true, replaceFrom: "# TODO_1B: 选择抵达后的动作", replaceTo: "collect()" },
          { id: "task2-route", label: "任务 2 · 安全向北", code: "turn_left()\nwhile not at_gem():\n    if is_spike_ahead():\n        shield()\n    move()", help: "转向、判断尖刺并前进", block: true, guided: true, replaceFrom: "# TODO_2A: 选择安全路线", replaceTo: "turn_left()\nwhile not at_gem():\n    if is_spike_ahead():\n        shield()\n    move()" },
          { id: "task2-collect", label: "任务 2 · 采集", code: "collect()", help: "采集第二座宝石", block: true, guided: true, replaceFrom: "# TODO_2B: 选择抵达后的动作", replaceTo: "collect()" },
        ...
        translations: [["at_gem()", "是否在宝石格"], ["at_relay()", "是否在中继站"], ["is_spike_ahead()", "前方是否尖刺"], ["upload()", "上传任务数据"]]
      },
      start: { x: 1, y: 5 },
      startDir: "E",
      grid: [
      ...
      limit: 14,
      brief: {
        challenge: "完成能源搜救 Boss 关：采集两座宝石、穿过两处尖刺格，并带着至少 4 点能量完成上传。",
        intro: "这张长地图要求你同时管理程序结构与运行状态：",
        bullets: ["用循环压缩三段重复移动。", "只在尖刺格前开启护盾，避免浪费能量。", "上传前检查两座宝石和剩余能量。"],
    ...
    17: {
      ...
      pythonStudio: {
        ...
        title: "让采集计数跟着任务变化",
        taskBadge: "补全第 11 行",
        taskHtml: "程序已经采集两座宝石，但变量 <code>collected</code> 只记录到 1。补上第二次更新，让变量轨迹真实显示 <code>0 → 1 → 2</code>。",
        ...
        allowedFunctions: ["move", "collect", "at_gem"],
        targets: [
        ...
        templates: [
          ...
          { id: "collect", code: "collect()", help: "采集当前位置的宝石", snippet: "collect()" }
        ],
        translations: [["collected", "已采集数量"], ["=", "把右侧结果保存到左侧变量"], ["collected + 1", "在当前数量上增加 1"], ["at_gem()", "是否到达未采集宝石"]]
      },
      start: { x: 1, y: 2 },
      startDir: "E",
      grid: [
      ...
      limit: 7,
      brief: {
        challenge: "采集两座宝石，并用变量 collected 记录已经完成的采集数量。",
        intro: "变量给会变化的状态起名字：",
    ...
    18: {
      ...
      pythonStudio: {
        ...
        allowedFunctions: ["move", "turn_left", "turn_right", "shield", "collect", "upload", "at_gem", "at_relay", "energy_remaining"],
        targets: [
    ...
    19: {
      ...
      pythonStudio: {
        ...
        allowedFunctions: ["Explorer", "Flyer", "move", "collect", "upload", "at_gem", "at_relay"],
        targets: [
          { search: "scout.collect()", occurrence: 1, hint: "让 Explorer 实例 Nova 负责采集" }
        ],
        conceptChecks: [
          { pattern: "\\bnova\\s*=\\s*Explorer\\(", message: "需要用 Explorer 创建 Nova" },
          { pattern: "\\bscout\\s*=\\s*Flyer\\(", message: "需要用 Flyer 创建 Scout" },
          { pattern: "\\bnova\\.collect\\(\\)", message: "采集方法应该由 Explorer 实例 Nova 调用" }
        ],
        stateChecks: [
          { kind: "object", name: "Nova", property: "type", equals: "Explorer", message: "Nova 的类型应该是 Explorer" },
          { kind: "object", name: "Scout", property: "type", equals: "Flyer", message: "Scout 的类型应该是 Flyer" },
          { kind: "object-action", name: "Nova", type: "Explorer", action: "collect", message: "采集动作需要记录在 Nova 的对象档案中" }
        ],
        statePanel: {
          title: "对象能力档案",
          description: "点语法左侧是负责人，右侧是它请求执行的方法。",
          objects: [
            { name: "Nova", label: "采集探测员", expectedType: "Explorer", targetEnergy: 2 },
            { name: "Scout", label: "空中侦察员", expectedType: "Flyer", targetEnergy: 4 }
          ]
        },
        templates: [
          { id: "nova-collect", label: "交给 Nova", code: "nova.collect()", help: "Explorer 具备采集能力", recommended: true, replaceFrom: "scout.collect()", replaceTo: "nova.collect()" },
          { id: "scout-collect", label: "仍交给 Scout", code: "scout.collect()", help: "Flyer 不具备采集能力", snippet: "scout.collect()" },
          { id: "nova-upload", code: "nova.upload()", help: "由 Explorer 上传任务数据", snippet: "nova.upload()" }
        ],
        translations: [["Explorer", "探测员类型：移动、采集、上传"], ["Flyer", "飞板类型：移动、扫描"], ["nova.collect()", "让 Nova 执行采集"], ["scout.collect()", "让 Scout 执行采集（能力不匹配）"]]
      },
      start: { x: 1, y: 2 },
      startDir: "E",
      grid: [
      ...
      brief: {
        ...
        intro: "对象把状态和能力组织在一起：",
        bullets: ["Explorer 可以移动、采集和上传。", "Flyer 可以移动与扫描，但不能采集。", "nova.collect() 表示由 nova 这个实例负责采集。"],
        goal: "修复能力不匹配，使用 Nova 完成采集和上传。"
      }
    },
    20: {
      ...
      pythonStudio: {
        ...
        allowedFunctions: ["Explorer", "move", "collect", "upload", "turn_right", "at_gem", "at_relay"],
        targets: [
    ...
    21: {
      ...
      pythonStudio: {
        ...
        title: "让同一个工具适应两种距离",
        taskBadge: "修复第 8 行",
        taskHtml: "<code>move_steps(steps)</code> 会按照参数移动。第一段到宝石需要 2 格，第二段到中继站需要 4 格；修复第二次调用的实参。",
    ...
    22: {
      ...
      pythonStudio: {
        ...
        title: "把安全判断交回给 if",
        taskBadge: "修复第 2 行",
        taskHtml: "<code>safe_ahead()</code> 应在前方安全时返回 <code>True</code>。起始程序把判断方向写反，导致尖刺格出现时没有开启护盾。",
        railEyebrow: "函数回答，调用者决策",
        railTitle: "返回值流向图",
        initialFeedback: "先运行起始程序，观察 safe_ahead() 返回 True 后，if 为什么跳过 shield()。",
        starterSource: [
          "def safe_ahead():",
          "    return is_spike_ahead()",
          "",
          "move()",
          "if not safe_ahead():",
          "    shield()",
        ...
        allowedFunctions: ["is_spike_ahead", "move", "shield", "collect"],
        targets: [
          { search: "return is_spike_ahead()", occurrence: 1, hint: "安全代表前方不是尖刺格" }
        ],
        conceptChecks: [
          { pattern: "\\breturn\\s+not\\s+is_spike_ahead\\(\\)", message: "safe_ahead() 需要返回“前方不是尖刺格”" },
          { pattern: "\\bif\\s+not\\s+safe_ahead\\(\\):", message: "调用处需要使用返回值决定是否开盾" }
        ],
        stateChecks: [
          { kind: "function-return", name: "safe_ahead", equals: false, message: "尖刺格前 safe_ahead() 应真实返回 False" }
        ],
        statePanel: {
          title: "返回值流向图",
        ...
        templates: [
          { id: "return-safe", label: "返回不是尖刺", code: "return not is_spike_ahead()", help: "尖刺时返回 False", recommended: true, replaceFrom: "return is_spike_ahead()", replaceTo: "return not is_spike_ahead()" },
          { id: "return-hazard", label: "直接返回尖刺", code: "return is_spike_ahead()", help: "含义与函数名相反", snippet: "return is_spike_ahead()" },
          { id: "use-return", code: "if not safe_ahead():", help: "使用函数返回值进行判断", snippet: "if not safe_ahead():" }
        ],
        translations: [["return", "把函数算出的结果交回调用位置"], ["safe_ahead()", "询问前方是否安全"], ["not is_spike_ahead()", "前方不是尖刺格"], ["if not safe_ahead()", "不安全时执行护盾动作"]]
      },
      start: { x: 1, y: 2 },
      startDir: "E",
      grid: [
      ...
      limit: 6,
      brief: {
        challenge: "让安全查询函数返回正确布尔值，并由 if 使用它穿越尖刺格。",
        intro: "返回值是函数交回来的答案：",
        bullets: ["查询函数不直接移动或开盾。", "return 决定调用表达式得到什么值。", "if 使用这个值选择是否执行动作。"],
        goal: "修复返回方向，安全抵达宝石并采集。"
      }
    },
    23: {
      ...
      pythonStudio: {
        ...
        title: "补回任务清单里的第三段距离",
        taskBadge: "修复第 5 行",
        taskHtml: "距离列表会先用索引读取第 0 项，再遍历剩余元素。起始清单少了第三座宝石前的 3 格距离，请补回漏项。",
        railEyebrow: "顺序就是执行计划",
        railTitle: "多目标任务清单",
        initialFeedback: "先运行起始程序。列表只有两项，所以循环处理完第二座宝石后会跳过第三座。",
        starterSource: [
          "def move_steps(steps):",
          "    for _ in range(steps):",
          "        move()",
        ...
        statePanel: {
          ...
          collections: [
            { name: "target_distances", label: "宝石距离列表", target: [2, 2, 3] }
          ],
          functions: [
        ...
        templates: [
          ...
          { id: "short-list", label: "保留两项", code: "target_distances = [2, 2]", help: "会漏掉第三座宝石", snippet: "target_distances = [2, 2]" },
          { id: "first-index", code: "target_distances[0]", help: "索引 0 表示第一项", snippet: "target_distances[0]" }
        ],
        translations: [["[2, 2, 3]", "按顺序保存三段宝石距离"], ["target_distances[0]", "读取列表第一项"], ["target_distances[1:]", "读取第一项之后的全部元素"], ["for distance in", "依次把每个元素交给 distance"]]
      },
      start: { x: 1, y: 2 },
      startDir: "E",
      grid: [
      ...
      limit: 10,
      brief: {
        challenge: "用一个有序距离列表依次采集三座宝石，再前往中继站。",
        intro: "列表是有顺序的任务清单：",
        bullets: ["索引 0 读取第一项。", "切片 [1:] 表示剩余全部元素。", "for 会按列表顺序逐个处理。"],
        goal: "补回第三段距离，让任务清单覆盖全部宝石。"
      }
    },
    24: {
      ...
      brief: {
        ...
        goal: "修复越界，完成三座宝石与最终上传，生成数据与函数任务包。"
      }
    },
    25: {
      ...
      pythonStudio: {
        ...
        title: "修复尖刺地形对应的处理策略",
        taskBadge: "修复第 1 行",
        taskHtml: "字典把地形字符映射到处理动作。起始规则把尖刺格 <code>H</code> 对应成了 <code>move</code>，导致查询结果无法开启护盾。",
        railEyebrow: "用键找到规则",
        railTitle: "地形策略字典",
        ...
        statePanel: {
          ...
          variables: [
            { name: "hazard_action", label: "尖刺格查询结果", initial: "未查询", target: "shield" }
          ]
        },
        templates: [
          { id: "shield-rule", label: "H 对应护盾", code: "\"H\": \"shield\"", help: "尖刺格前开启护盾", recommended: true, replaceFrom: "\"H\": \"move\"", replaceTo: "\"H\": \"shield\"" },
        ...
        translations: [["terrain_actions", "保存地形处理规则的字典"], ["\"H\": \"shield\"", "尖刺地形对应护盾策略"], ["terrain_actions[\"H\"]", "查找键 H 对应的值"], ["hazard_action", "本次查询得到的动作名称"]]
      },
      start: { x: 1, y: 2 },
      startDir: "E",
      grid: [
      ...
      limit: 9,
      brief: {
        challenge: "通过规则字典查找尖刺地形对应的动作，安全采集并上传。",
        intro: "字典把名称与规则配对：",
    ...
    26: {
      ...
      pythonStudio: {
        ...
        allowedFunctions: ["build_world", "move", "collect", "upload", "at_gem", "at_relay"],
        targets: [
          { search: "\"B\", \"g\", \"g\", \"_\"", occurrence: 1, hint: "把 row2 中索引 5 的 g 改成 R" }
        ],
        conceptChecks: [
          { pattern: "row2\\s*=\\s*\\[[^\\n]*[\"']B[\"'][^\\n]*[\"']R[\"']", message: "row2 必须同时编码宝石 B 与中继站 R" },
    ...
    27: {
      ...
      pythonStudio: {
        ...
        allowedFunctions: ["build_world", "place_tile", "remove_tile", "pair_portals", "move", "collect", "upload", "at_gem"],
        targets: [
        ...
        templates: [
          { id: "portal-on-island", label: "出口放到第二座岛", code: "pair_portals(3, 1, 5, 2)", help: "传送后前方是宝石", recommended: true, replaceFrom: "pair_portals(3, 1, 5, 1)", replaceTo: "pair_portals(3, 1, 5, 2)" },
    ...
    28: {
      ...
      pythonStudio: {
        ...
        allowedFunctions: ["RescueKit", "move", "collect", "upload", "at_gem", "at_relay"],
        targets: [
    ...
    29: {
      ...
      pythonStudio: {
        ...
        taskBadge: "修复第 8 行",
        taskHtml: "Nova 与 Carrier 有独立位置和状态。<code>Spaceship</code> 可以移动与上传，但不能采集；请把宝石任务交回 Explorer。",
        railEyebrow: "每个对象保留自己的状态",
        railTitle: "协作对象档案",
        initialFeedback: "先运行起始程序。Carrier 会在自己的位置移动到宝石，但能力边界会阻止它采集。",
        starterSource: [
          "nova = Explorer(\"Nova\", 8, 1, 2, \"E\")",
        ...
        allowedFunctions: ["Explorer", "Spaceship", "move", "collect", "upload", "at_gem", "at_relay"],
        targets: [
          { search: "carrier.collect()", occurrence: 1, hint: "让 Explorer 实例 Nova 执行采集" }
        ],
        conceptChecks: [
          { pattern: "\\bnova\\.collect\\(\\)", message: "采集必须交给 Explorer 实例 Nova" },
          { pattern: "\\bcarrier\\.upload\\(\\)", message: "上传应由 Spaceship 实例 Carrier 完成" }
        ],
        stateChecks: [
          { kind: "object-action", name: "Nova", type: "Explorer", action: "collect", message: "Nova 需要留下采集动作记录" },
        ...
        statePanel: {
          ...
          objects: [
            { name: "Nova", label: "采集负责人", expectedType: "Explorer", targetPosition: [3, 2], targetEnergy: 6, showPosition: true },
            { name: "Carrier", label: "上传负责人", expectedType: "Spaceship", targetPosition: [5, 1], targetEnergy: 2, showPosition: true }
          ]
        },
        templates: [
          { id: "nova-collect-role", label: "交给 Nova 采集", code: "nova.collect()", help: "Explorer 具备采集能力", recommended: true, replaceFrom: "carrier.collect()", replaceTo: "nova.collect()" },
        ...
        translations: [["nova", "拥有独立位置的 Explorer 实例"], ["carrier", "拥有独立位置的 Spaceship 实例"], ["nova.collect()", "由 Nova 完成采集"], ["carrier.upload()", "由 Carrier 完成上传"]]
      },
      start: { x: 1, y: 2 },
      startDir: "E",
      grid: [
      ...
      limit: 8,
      brief: {
        challenge: "让 Nova 负责采集、Carrier 负责上传，并保留各自的位置与能量状态。",
        intro: "多对象协作先明确能力边界：",
    ...
    31: {
      ...
      pythonStudio: {
        ...
        taskBadge: "修复第 4 行",
        taskHtml: "原创关卡字典声明有 2 座宝石，但地图实际只编码了 1 座。修复 <code>gems</code> 字段，通过校验后生成并走通世界。",
        ...
        allowedFunctions: ["validate_world", "build_world", "move", "collect", "upload", "at_gem", "at_relay"],
        targets: [
          { search: "\"gems\": 2", occurrence: 1, hint: "地图实际只有 1 座宝石" }
        ],
        conceptChecks: [
          { pattern: "[\"']gems[\"']\\s*:\\s*1", message: "schema 的 gems 字段应与地图中的 1 座宝石一致" },
        ...
        stateChecks: [
          { kind: "world-schema", property: "gems", equals: 1, message: "关卡 schema 的宝石数量仍不正确" }
        ],
        statePanel: {
          title: "原创关卡 v1",
          description: "schema 校验通过后，地图字段才进入世界生成器与参考解验证。",
          worlds: [
            { name: "level", label: "Relay Garden", targetGrid: ["_______", "_SgBgR_", "_______"], targetText: "schema 需声明 1 座宝石并要求上传" }
          ],
          collections: [
            { name: "level", label: "关卡 schema", target: { name: "Relay Garden", map: ["_______", "_SgBgR_", "_______"], gems: 1, upload: true } }
          ]
        },
        templates: [
          { id: "one-gem", label: "声明 1 座宝石", code: "\"gems\": 1", help: "与地图中的 B 数量一致", recommended: true, replaceFrom: "\"gems\": 2", replaceTo: "\"gems\": 1" },
          { id: "two-gems", label: "仍声明 2 座", code: "\"gems\": 2", help: "会被 schema 校验拒绝", snippet: "\"gems\": 2" },
          { id: "validate-level", code: "validate_world(level)", help: "比较字段、地图与成功条件", snippet: "validate_world(level)" }
        ],
        translations: [["schema", "机器可读取的关卡规则约定"], ["gems", "关卡要求的宝石数量"], ["upload", "是否要求到中继站上传"], ["validate_world", "检查规则与地图是否一致"]]
      },
      start: { x: 1, y: 1 },
      startDir: "E",
      grid: [
      ...
      brief: {
        ...
        intro: "发布关卡需要一份完整约定：",
        bullets: ["name 标识作品。", "map 描述世界。", "gems 与 upload 描述成功条件。"],
    ...
    32: {
      ...
      pythonStudio: {
        ...
        starterSource: [
          "world = {\"name\": \"Reconnection\", \"map\": [\"________\", \"_gggggR_\", \"_SgBggg_\", \"________\"], \"gems\": 1, \"upload\": True}",
        ...
        allowedFunctions: ["validate_world", "build_world", "Explorer", "Spaceship", "move", "wait", "collect", "upload", "at_gem", "at_relay"],
        targets: [
        ...
        conceptChecks: [
          ...
          { pattern: "\\bnova\\.collect\\(\\)", message: "Explorer 需要完成采集职责" },
          { pattern: "\\bcarrier\\.wait\\(\\)", message: "Carrier 上传前需要留下同步等待证据" },
          { pattern: "\\bcarrier\\.upload\\(\\)", message: "Spaceship 需要完成上传职责" }
        ],
        stateChecks: [
          { kind: "world-schema", property: "gems", equals: 1, message: "世界 schema 需要通过校验" },
          { kind: "object-action", name: "Nova", type: "Explorer", action: "collect", message: "Nova 需要留下采集证据" },
        ...
        statePanel: {
          ...
          objects: [
            { name: "Nova", label: "采集对象", expectedType: "Explorer", targetPosition: [3, 2], targetEnergy: 6, showPosition: true },
  ...
  const extendedLessonSpecs = {
    33: { mode: "search-scanner", toolTitle: "线性搜索扫描器", kind: "search", options: ["寻找最近宝石", "寻找安全宝石", "寻找指定坐标"], evidence: ["按顺序检查候选目标", "条件不匹配就继续", "列表结束后处理找不到"] },
    ...
    39: { mode: "path-cost", toolTitle: "路线代价比较器", kind: "cost", options: ["最少步数", "最低能量", "最低风险"], evidence: ["最短不一定代价最低", "尖刺格改变路线价值", "评价前必须说明指标"] },
    ...
    43: { mode: "counterexample-lab", toolTitle: "贪心反例实验室", kind: "counterexample", options: ["近处低收益", "远处高收益", "尖刺捷径"], evidence: ["反例必须满足算法规则", "比较贪心结果与全局结果", "失败说明算法适用边界"] },
    ...
    47: { mode: "test-matrix", toolTitle: "策略测试矩阵", kind: "test", options: ["短直线", "尖刺捷径", "多目标地图", "无解地图"], evidence: ["不能只用一张图证明策略", "同时记录成功率、步数和能量", "失败用例同样是证据"] },
  ...
  function buildMission(values, index) {
    ...
    const routeChoices = isRouteComparison
      ? [
          ...
          },
          {
            id: "detour",
            label: "路线 B · 绕行",
            summary: "向南 1 格，向东 2 格，再向北 1 格到达宝石。",
    ...
    const filteredAllowed = baseAllowed
      ...
      .filter((command) => command !== "shield" || row.knowledgeText.includes("尖刺") || row.knowledgeText.includes("能量") || row.no >= 39)
      .filter((command) => command !== "ifHazardShield" || row.knowledgeText.includes("尖刺") || row.knowledgeText.includes("条件") || row.no >= 39);
    ...
    return {
      ...
      coordinateTargets: isCoordinateScanner
        ? sim.beacons.map((point, targetIndex) => ({
            ...
            label: `宝石 ${String.fromCharCode(65 + targetIndex)}`,
            x: point.x,
            y: point.y
          }))
        : undefined,
