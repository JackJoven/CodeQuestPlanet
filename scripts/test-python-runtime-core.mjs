...
assert.match(pageSource, /id="programReplaceBtn"[^>]*>替换<\/button>/);
assert.match(pageSource, /id="programInsertBtn"[^>]*>添加<\/button>/);
assert.match(pageSource, /id="programCancelBtn"[^>]*>取消<\/button>/);
assert.match(appSource, /data-select-step=/);
assert.match(appSource, /请选择替换或添加/);
assert.match(appSource, /选择要添加的指令/);
assert.doesNotMatch(appSource, /请选择要添加的指令/);
assert.doesNotMatch(appSource, /请选择本条指令下要添加的指令/);
assert.match(appSource, /is-edit-confirmed/);
assert.match(pageSource, /再运行你的程序[\s\S]*?world-view-help/);
assert.match(appSource, /targetCanvas\.addEventListener\("wheel"/);
assert.match(appSource, /targetCanvas\.addEventListener\("contextmenu"/);
assert.match(appSource, /targetCanvas\.addEventListener\("dblclick"/);
assert.match(appSource, /event\.button === 2 \|\| \(event\.button === 0 && event\.shiftKey\)/);
